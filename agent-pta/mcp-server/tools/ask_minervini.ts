...
export function registerMinerviniTools(server: McpServer) {
  server.registerTool(
    "manage_minervini_risk",
    {
      title: "Manage Minervini Risk",
      description: "Strict risk management tool. Validates trade setups, calculates missing parameters, and updates global rules based on pta_live_risk.",
      inputSchema: {
        action: z.enum(["VALIDATE_TRADE", "UPDATE_PARAMETERS"]).describe("The action to perform"),
        // UPDATE parameters
        ...
        current_price: z.string().or(z.number()).optional().describe("Current price of the asset"),
    ...
    async (params: any) => {
      try {
        if (params.action === "UPDATE_PARAMETERS") {
        ...
        if (params.action === "VALIDATE_TRADE") {
            if (!params.symbol || !params.current_price) {
                return { content: [{ type: "text", text: "Error: symbol and current_price are required for VALIDATE_TRADE." }], isError: true };
            }
        ...
        return { content: [{ type: "text", text: `Error in manage_minervini_risk: ${err.message}` }], isError: true };
      }
    }
  );
}
