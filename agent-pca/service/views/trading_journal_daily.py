import os
import httpx
import logging
import datetime
from pathlib import Path
from collections import defaultdict
import duckdb
from fastapi import HTTPException

logger = logging.getLogger("pca.views.trading_journal_daily")
postgrest_url = os.environ.get("POSTGREST_URL", "http://postgrest:3000")
PARQUET_BASE = Path(os.environ.get("PARQUET_BASE_PATH", "/parquet"))

def _parquet_path(symbol: str, timeframe: str) -> Path:
    return PARQUET_BASE / symbol.upper() / f"{timeframe}.parquet"

def get_chart_data_daily(symbol: str, timeframe: str, limit: int) -> dict:
    columns = ["timestamp", "open", "high", "low", "close", "volume"]
    
    try:
        r = httpx.get(f"{postgrest_url}/pta_execution_log?event_type=in.(FILL,CASH_TRANSFER)&order=created_at.asc&limit=50000")
        r.raise_for_status()
        exec_logs = r.json()
        
        r_rates = httpx.get(f"{postgrest_url}/exchange_rates?base_currency=eq.EUR&order=date.asc&limit=50000")
        r_rates.raise_for_status()
        rates_data = r_rates.json()
    except Exception as e:
        logger.exception("Failed to fetch data from DB: %s", e)
        raise HTTPException(status_code=500, detail="Failed to fetch stats data")

    if not exec_logs:
        return {"symbol": symbol, "timeframe": timeframe, "count": 0, "columns": columns, "data": []}

    # Prepare FX rates
    fx = defaultdict(dict)
    for r_item in rates_data:
        curr = r_item["target_currency"]
        d = r_item["date"][:10]
        fx[curr][d] = float(r_item["rate"])
        
    def get_fx(curr, target_date):
        if curr == "EUR" or not curr: return 1.0
        dates = sorted([d for d in fx[curr].keys() if d <= target_date])
        if dates: return fx[curr][dates[-1]]
        all_dates = sorted(fx[curr].keys())
        if all_dates: return fx[curr][all_dates[0]]
        return 1.0

    # Load Prices
    ticker_currency = {}
    for ev in exec_logs:
        if ev.get("ticker") and ev.get("currency"):
            ticker_currency[ev["ticker"]] = ev["currency"]
    tickers = set(ticker_currency.keys())
    prices = defaultdict(dict)
    
    try:
        db = duckdb.connect()
        for t in tickers:
            p = _parquet_path(t, "1D")
            if p.exists():
                try:
                    res = db.execute(f"SELECT timestamp, close FROM read_parquet('{p}') WHERE close IS NOT NULL").fetchall()
                    for row in res:
                        ts = row[0]
                        if ts > 10000000000:
                            ts = ts / 1000.0
                        dt_str = datetime.datetime.fromtimestamp(ts, tz=datetime.timezone.utc).strftime("%Y-%m-%d")
                        prices[t][dt_str] = float(row[1])
                except Exception as e:
                    logger.error(f"Error reading {p}: {e}")
        db.close()
    except Exception as e:
        logger.error(f"Duckdb error: {e}")

    def get_price(t, target_date):
        dates = sorted([d for d in prices[t].keys() if d <= target_date])
        if dates: return prices[t][dates[-1]]
        return 0.0

    # State
    current_cash = 0.0
    positions = defaultdict(float)
    
    start_dt = datetime.datetime.fromisoformat(exec_logs[0]["created_at"].replace("Z", "+00:00")).date()
    end_dt = datetime.datetime.utcnow().date()
    
    data = []
    log_idx = 0
    num_logs = len(exec_logs)
    curr_date = start_dt
    
    while curr_date <= end_dt:
        curr_date_str = curr_date.strftime("%Y-%m-%d")
        
        # PnL logic specifically for daily view: sum PnL of all trades on this day
        daily_pnl = 0.0
        
        while log_idx < num_logs:
            ev = exec_logs[log_idx]
            ev_dt = datetime.datetime.fromisoformat(ev["created_at"].replace("Z", "+00:00")).date()
            if ev_dt > curr_date:
                break
                
            action = ev.get("action")
            qty = float(ev.get("quantity") or 0.0)
            price = float(ev.get("price") or 0.0)
            comm = float(ev.get("commission") or 0.0)
            ticker = ev.get("ticker")
            currency = ev.get("currency")
            
            rate = get_fx(currency, ev_dt.strftime("%Y-%m-%d"))
            val_eur = (qty * price) / rate
            comm_eur = comm / rate
            
            if ev["event_type"] == "CASH_TRANSFER":
                if action == "DEPOSIT":
                    current_cash += (qty / rate)
                elif action == "WITHDRAW":
                    current_cash -= (qty / rate)
            elif ev["event_type"] == "FILL":
                if action == "BUY":
                    current_cash -= (val_eur + comm_eur)
                    positions[ticker] += qty
                elif action == "SELL":
                    current_cash += (val_eur - comm_eur)
                    positions[ticker] -= qty
                    # For PnL calculation we approximate realized PnL per trade.
                    # Wait, true PnL requires cost basis. 
                    # If symbol is PNL we can fetch from trade history later, but for simplicity:
                    pass
                    
                if positions[ticker] <= 0.0001:
                    del positions[ticker]
                    
            log_idx += 1
            
        # If PnL was requested, we should probably fetch trade history.
        # Let's handle trade history if symbol == $STATS.PNL
        
        # End of day valuation
        asset_value = 0.0
        for t, q in positions.items():
            px = get_price(t, curr_date_str)
            r = get_fx(ticker_currency.get(t, "EUR"), curr_date_str)
            asset_value += (q * px) / r
            
        nav = current_cash + asset_value
        cash_quote = (current_cash / nav * 100.0) if nav > 0 else 0.0
        
        if symbol == "$STATS.NAV":
            val = nav
        elif symbol == "$STATS.ASSETS_VALUE":
            val = asset_value
        elif symbol == "$STATS.CASH_QUOTE":
            val = cash_quote
        else:
            val = 0.0 # Will be populated by PnL logic if needed

        # Only append if we actually have data, to prevent leading zeroes
        if current_cash != 0 or asset_value != 0:
            ts_sec = int(datetime.datetime.combine(curr_date, datetime.time(23, 59, 59)).timestamp())
            data.append([ts_sec, val, val, val, val, 0])
            
        curr_date += datetime.timedelta(days=1)

    # Special handling for PNL - replace the data values with actual trade history realized PnL grouped by day
    if symbol == "$STATS.PNL":
        # Reset data
        data = []
        try:
            r = httpx.get(f"{postgrest_url}/pta_trade_history?order=close_time.asc&limit=50000")
            r.raise_for_status()
            trades = r.json()
        except Exception:
            trades = []
            
        daily_pnl_map = defaultdict(float)
        for t in trades:
            if not t.get("close_time"): continue
            d_str = t["close_time"][:10]
            daily_pnl_map[d_str] += float(t.get("pnl_net", 0.0))
            
        # Rebuild daily data for PNL to match exact identical timestamps as NAV
        curr_date = start_dt
        while curr_date <= end_dt:
            d_str = curr_date.strftime("%Y-%m-%d")
            pnl_val = daily_pnl_map.get(d_str, 0.0)
            
            # Use same timezone/time combination to match sync
            ts_sec = int(datetime.datetime.combine(curr_date, datetime.time(23, 59, 59)).timestamp())
            data.append([ts_sec, pnl_val, pnl_val, pnl_val, pnl_val, 0])
            curr_date += datetime.timedelta(days=1)

    if limit > 0:
        data = data[-limit:]

    return {
        "symbol": symbol,
        "timeframe": timeframe,
        "count": len(data),
        "columns": columns,
        "data": data
    }
