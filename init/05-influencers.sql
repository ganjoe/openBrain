...
DECLARE
  clean_text text;
BEGIN
  clean_text := regexp_replace(query_text, '^[@]', '');
  RETURN QUERY
  SELECT
    t.username,
    t.screen_name,
    ...
    CASE
      WHEN lower(t.username) = lower(clean_text) AND (t.notes IS NOT NULL OR t.embedding IS NOT NULL) THEN 450
      WHEN lower(coalesce(t.screen_name, '')) = lower(clean_text) AND (t.notes IS NOT NULL OR t.embedding IS NOT NULL) THEN 420
      WHEN lower(t.username) = lower(clean_text) THEN 400
      WHEN lower(coalesce(t.screen_name, '')) = lower(clean_text) THEN 380
      WHEN t.username ILIKE clean_text || '%' THEN 300
      WHEN t.screen_name ILIKE clean_text || '%' THEN 250
      WHEN t.username ILIKE '%' || clean_text || '%' THEN 200
      WHEN t.screen_name ILIKE '%' || clean_text || '%' THEN 150
      WHEN t.notes ILIKE '%' || clean_text || '%' THEN 100
      ELSE 0
  ...
  WHERE t.is_active = TRUE
    AND (
      ...
      OR t.username ILIKE '%' || clean_text || '%'
      OR t.screen_name ILIKE '%' || clean_text || '%'
      OR t.notes ILIKE '%' || clean_text || '%'
    )
