...
const AGENT_ID       = config.agent.id;           // e.g. "pca" — never hardcoded
...
async function main() {
  ...
  await mqttClient.subscribeAsync("agents/stock-data/events", { qos: 1 });
  console.log(`📡 Subscribed: ${INBOX_TOPIC} | ${MCP_REQ_TOPIC} | ${MCP_RESP_TOPIC} | system/config/provider | system/config/context_limit | agents/stock-data/events`);
